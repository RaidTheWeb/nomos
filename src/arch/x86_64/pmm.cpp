...
namespace NArch {
    ...
    void PMM::setup(void) {
        ...
        this->zone.addr = largestaddr;
        this->zone.size = largestsize;

        for (size_t i = 0; i < ALLOCLEVEL; i++) {
            this->zone.freelist[i] = NULL;
        }
        ...
        while (size > biggestblock) {
            ...
            struct block *block = (struct block *)(this->zone.addr + (i * biggestblock)); // Pick an area.
        ...
        void *test1 = this->alloc(4096);
        this->free(test1);
        void *test2 = this->alloc(4096);
        this->free(test2);
    ...
    void *PMM::alloc(size_t size) {
        ...
        while (found > level) { // While we haven't yet split down to the comfiest level.
            struct block *block = this->zone.freelist[found]; // Grab the head of the freelist.
            this->zone.freelist[found] = block->next; // We're planning to consume this block, so move the freelist onwards.
            ...
            block->next = this->zone.freelist[found - 1]; // This block will be just before the latest on the free list.
            second->next = block; // This block will be just behind the other block.
            this->zone.freelist[found - 1] = second; // Shove this on the head of the free list.
    ...
    void PMM::free(void *ptr) {
        ...
        while (level < ALLOCLEVEL - 1) {
            ...
            struct block **prev = &this->zone.freelist[level];
        ...
        block->next = this->zone.freelist[level];
        this->zone.freelist[level] = block;
    }
}
